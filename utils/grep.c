/*This is a modified version, used to output spans and delta between matches. */
/*We only changed this file from the generic grep library. To compile, */
   /*download grep source code and replace grep.c with this one.*/
...
// ADDED - includes for time measurement
#include <sys/time.h>
...
// ADDED - timeval structs to measure time
       ...
       cur = b + match_size)
    {
      ...
      if (match_size == 0)
      ...
      else
        {
          ...
          // ADDED - stor the match size and offset in the buffer and print it later
          fwrite_errno (b, 1, match_size);
          pr_sgr_end_if (match_color);

          // calculate the offset to the current block start plus the offset in the current block
          int match_offset_in_file = file_offset - (buflim - bufbeg - 1) + b - bufbeg;
            //printf("^^^ file_offset = %lu inc %lu lim-beg = %lu bufbeg = %08X, b = %08X, total %d\n", file_offset, buflim-bufbeg, bufbeg, b, match_offset_in_file);
          if (match_offset_in_file >= 0) {
            gettimeofday(&t2, NULL);
            elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000 * 1000;
            elapsedTime += (t2.tv_usec -t1.tv_usec);
            // printing a new line here to ensure the magic is at the start of a line
            printf("\n%s{\"match\": \"", MATCH_MAGIC);
            for (char* ch = b; ch < b + match_size; ch++)
            {
              printf("%c", *ch);
            }
            printf("\", \"span\": [%lu,%lu], \"time\": %lu}\n", match_offset_in_file, match_offset_in_file + match_size - 1, elapsedTime);
...
grep (int fd, struct stat const *st, bool *ineof)
{
  ...
  // ADDED - grep doesn't store the match offset in the file so we add another variable for it
  file_offset = 0;
  ...
  // ADDED - update the file offset after each chunk
  file_offset += (buflim - bufbeg -1);
  ...
  for (bool firsttime = true; ; firsttime = false)
    {
      ...
      // ADDED - update file offset
      file_offset += (buflim - bufbeg - save);
    }
  if (residue)
    {
      *buflim++ = eol;
...
main (int argc, char **argv)
{
  ...
  elapsedTime += (t2.tv_usec -t1.tv_usec);
  // printing a new line here to ensure the magic is at the start of a line
  printf("\n%s{\"match\": \"EOF\", \"span\":  [-1,-1], \"time\": %lu}\n", MATCH_MAGIC, elapsedTime);
