...
def test_create_cube_with_correct_dim(analysis):
    ...
    assert analysis.cube.data.shape == (3, 2, 3)
...
def test_logs_mapped_to_parameter_cube_points(analysis):
    results_file_for_point = analysis.cube.data[2, 0, 1]['results']
    assert results_file_for_point == EXPECTED_OUTPUT + "logs/modified_grep_easy-protein-patterns_10e5.csv"
    ...
    with open(EXPECTED_OUTPUT + "logs//metafile.csv") as mf:
        ...
        for row in reader:
            if row['filename'] == basename(results_file_for_point):
                assert row['alg'] == basename(config.ALGORITHMS[2])
